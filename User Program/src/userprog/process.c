...
process_execute (const char *file_name) 
{
  ...
  /* Copy the first word of FILE_NAME. */
  name_copy = malloc(15);
  if (name_copy == NULL) {
  ...
  if (space = strstr(fn_copy, " ")) {
    strlcpy(name_copy, fn_copy, space - fn_copy + 1);
  } else {
    strlcpy(name_copy, fn_copy, 15);
  }
  ...
  sema_down(&child->exec);
  /* If unable to load, return error. */
  if (!child->success) 
    return TID_ERROR;
  return tid;
}
...
start_process (void *file_name_)
{
  ...
  thread_current()->wait_status->success = success;
  sema_up(&thread_current()->wait_status->exec);
  if (!success) {
...
process_exit (void)
{
  ...
  int i;
  for (i = 0; i < 128; i++) {
    file_close(thread_current()->descriptors[i]);
  }
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  size_t len = 0;
  char *strings = *esp;
  for (; token != NULL && len + strlen(token) + 6 + index < PGSIZE;
       token = strtok_r (NULL, " ", &save_ptr)) {
    strings -= strlen(token) + 1;
    memcpy(strings, token, strlen(token) + 1);
    len += strlen(token) + 1;
    index++;
  }
  char **pointers = (char **)((uintptr_t)strings - ((uintptr_t)strings % 4));
  pointers -= 1;
  int j;
  for (j = 0; j < index; j++) {
    pointers -= 1;
